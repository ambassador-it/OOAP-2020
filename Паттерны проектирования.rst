...
Для классов ``A``, ``B`` и ``С`` из `файла`__ (abs_class.py) создайте базовый — абстрактный класс Base(ABC). Наследуйте его классами A, B C и проверьте работу их работу.

.. __: {filename}/https://github.com/BosenkoTM/OOAP-2020/blob/master/code/lab12/abs_class.py
...
.. code-block:: bash
    ...
    wget -qO - --post-file=<filename> http://ip:port/adapter.py/<name>
